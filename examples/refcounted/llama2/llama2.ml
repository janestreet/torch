...
let transformer vs ~model_args =
  ...
  Layer.of_fn (fun tokens ->
    Tensor.with_rc_scope_tensor (fun () ->
      ...
      in
      let h =
        List.fold__local__local layers ~init:h ~f:(fun h layer -> (layer ~freqs_cis) h)
      in
...
let sample ~transformer ~dataset ~device =
  ...
  Tensor.with_rc_scope (fun () ->
    ...
    let _, sampled_ys =
      ...
      |> List.fold__global__local ~init:(input, []) ~f:(fun (input, sampled_ys) _idx ->

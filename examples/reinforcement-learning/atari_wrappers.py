...
class WarpFrame(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(self.res, self.res, 1), dtype="uint8"
        )

    def observation(self, obs):
        frame = np.dot(
            obs.astype("float32"), np.array([0.299, 0.587, 0.114], "float32")
        )
        frame = np.array(
            Image.fromarray(frame).resize(
                (self.res, self.res), resample=Image.BILINEAR
            ),
            dtype=np.uint8,
        )
...
class FrameStack(gym.Wrapper):
    def __init__(self, env, k):
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(shp[0], shp[1], k), dtype="uint8"
        )
...
def make(env_name, num_processes):
    envs = SubprocVecEnv([make_env(env_name, 1337, i) for i in range(num_processes)])
    return envs

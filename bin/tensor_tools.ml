...
let image_to_tensor image_src pytorch_dst resize =
  ...
  in
  let tensor =
    if Stdlib.Sys.is_directory image_src
...
let () =
  ...
  let npz_to_pytorch_cmd =
    let npz_src =
      Arg.(
        required & pos 0 (some string) None & info [] ~docv:"SRC" ~doc:"Npz source file")
    in
    let pytorch_dst =
      Arg.(
        required
        & pos 1 (some string) None
        & info [] ~docv:"DEST" ~doc:"PyTorch destination file")
    in
  ...
  let image_to_tensor =
    ...
    let image_src =
      Arg.(
        ...
        & info [] ~docv:"SRC" ~doc:"Image source file or directory")
    in
    let pytorch_dst =
      Arg.(
        required
  ...
  let pytorch_to_npz_cmd =
    let pytorch_src =
      Arg.(
        required
        & pos 0 (some string) None
        & info [] ~docv:"SRC" ~doc:"PyTorch source file")
    in
    let npz_dst =
      Arg.(
        required
        & pos 1 (some string) None
        & info [] ~docv:"DEST" ~doc:"Npz destination file")
    in

...
tensor at_tensor_of_data(void *vs, int64_t *dims, int ndims,
...
void at_copy_data(tensor tensor, void *vs, int64_t numel,
                  int elt_size_in_bytes) {
  PROTECT(
...
void at_shape(tensor t, int *dims) {
  PROTECT(int i = 0; for (int dim : t->sizes()) dims[i++] = dim;)
}
...
int at_device(tensor tensor) {
  PROTECT(auto device = tensor->device(); if (device.is_cpu()) return -1;
          return device.index();)
}
...
void at_set_data(tensor dst, tensor src) { PROTECT(dst->set_data(*src);) }

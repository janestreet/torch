...
let load_image_no_resize_and_crop filename =
  Image.load_image filename |> Or_error.ok_exn |> normalize
;;

let load_images ~dir = Image.load_images dir ~resize:(224, 224) |> normalize
...
let write_image tensor ~filename =
  ...
  Image.write_image tensor ~filename
;;
...
module Classes = struct
  ...
  let names =
    [| "tench, Tinca tinca"
     ...
     ; "platypus, duckbill, duckbilled platypus, duck-billed platypus, Ornithorhynchus \
        anatinus"
...
module Loader = struct
  ...
  let random_batch t ~batch_size = Image.Loader.random_batch t ~batch_size |> normalize
  let reset = Image.Loader.reset

  let next_batch t ~batch_size =
    Image.Loader.next_batch t ~batch_size |> Option.map ~f:normalize
  ;;
end

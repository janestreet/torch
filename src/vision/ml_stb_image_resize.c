...
CAMLprim value ml_stbir_resize(value in_ba, value in_w, value in_h, value out_ba,
                               value out_w, value out_h, value nchannels) {
  ...
  if (validate_dim(in_ba, in_w, in_h, nchannels, 1) &&
    ...
    result = stbir_resize_uint8(Caml_ba_data_val(in_ba), Int_val(in_w), Int_val(in_h), 0,
                                Caml_ba_data_val(out_ba), Int_val(out_w), Int_val(out_h),
                                0, Int_val(nchannels));
  else
    result = 0;

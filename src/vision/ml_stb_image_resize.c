...
CAMLprim value ml_stbir_resize(value in_ba, value in_w, value in_h,
  ...
  if (validate_dim(in_ba, in_w, in_h, nchannels, 1) &&
    ...
    result = stbir_resize_uint8(Caml_ba_data_val(in_ba), Int_val(in_w),
                                Int_val(in_h), 0, Caml_ba_data_val(out_ba),
                                Int_val(out_w), Int_val(out_h), 0,
                                Int_val(nchannels));
  else
    result = 0;
...
CAMLprim value ml_stbir_resize_bytecode(value *argv, int nargs) {
  return ml_stbir_resize(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5],
                         argv[6]);
}

...
CAMLprim value ml_stbi_mipmap(value img_in, value img_out) {
  ...
  unsigned char *pin, *pout, *pin0 = Caml_ba_data_val(Field(img_in, 5)),
...
CAMLprim value ml_stbi_mipmapf(value img_in, value img_out) {
  CAMLparam2(img_in, img_out);
  float *pin, *pout, *pin0 = Caml_ba_data_val(Field(img_in, 5)),
                     *pout0 = Caml_ba_data_val(Field(img_out, 5));
  assert(pin0 && pout0);
...
static void expblur1(unsigned char *ptr, int w, int h, int stride, int alpha) {
  OUTERLOOP(dst, ptr, h, stride) {
    BLUR0(dst[0], acc0);
    INNERLOOP(x, w, 1, { BLUR(dst[x + 0], acc0); });
  }

  OUTERLOOP(dst, ptr, w, 1) {
    BLUR0(dst[0], acc0);
    INNERLOOP(y, h, stride, { BLUR(dst[y + 0], acc0); });
  }
}

static void expblur(unsigned char *ptr, int w, int h, int channels, int stride,
                    float radius) {
  int i, alpha;
  float sigma;
  ...
  switch (channels) {
  case 1:
    expblur1(ptr, w, h, stride, alpha);
    break;
  case 2:
    expblur2(ptr, w, h, stride, alpha);
    break;
  case 3:
    expblur3(ptr, w, h, stride, alpha);
    break;
  case 4:
    expblur4(ptr, w, h, stride, alpha);
    break;
  default:
    abort();
  }

...
let load_images ?resize dir =
  if not (Stdlib.Sys.is_directory dir) then Printf.failwithf "not a directory %s" dir ();
  ...
  List.filter_map files ~f:(fun filename ->
    ...
    then (
      match load_image (Stdlib.Filename.concat dir filename) ?resize with
      | Ok image -> Some image
      | Error msg ->
  ...
  let read () =
    ...
    let load_tv tv =
      List.mapi classes ~f:(fun class_index class_dir ->
        ...
        images, labels)
      |> List.unzip
      |> fun (images, labels) -> Tensor.concat images ~dim:0, Tensor.concat labels ~dim:0
    in
    ...
    then None
      ...
      let batch =
        List.init batch_size ~f:(fun i ->
          ...
          load_image ?resize:t.resize (Stdlib.Filename.concat dir file) |> Or_error.ok_exn)
        |> Tensor.concat ~dim:0
      in
...
let resize tensor ~height ~width =
  ...
  | [ _; c; _; _ ] when c = 1 || c = 3 ->
    Tensor.to_list tensor |> List.map ~f:resize_one |> Tensor.concat ~dim:0
  | _ -> Printf.failwithf "unexpected shape %s" (Tensor.shape_str tensor) ()
;;

...
CAMLprim value ml_stbi_write_png(value filename, value w, value h, value comp, value ba) {
  ...
  if (validate_dim(ba, w, h, comp, 1))
    result = stbi_write_png(String_val(filename), Int_val(w), Int_val(h), Int_val(comp),
                            Caml_ba_data_val(ba), 0);
  else
    result = 0;
...
CAMLprim value ml_stbi_write_bmp(value filename, value w, value h, value comp, value ba) {
  ...
  if (validate_dim(ba, w, h, comp, 1))
    result = stbi_write_bmp(String_val(filename), Int_val(w), Int_val(h), Int_val(comp),
                            Caml_ba_data_val(ba));
  else
    result = 0;
...
CAMLprim value ml_stbi_write_tga(value filename, value w, value h, value comp, value ba) {
  ...
  if (validate_dim(ba, w, h, comp, 1))
    result = stbi_write_tga(String_val(filename), Int_val(w), Int_val(h), Int_val(comp),
                            Caml_ba_data_val(ba));
  else
    result = 0;
...
CAMLprim value ml_stbi_write_hdr(value filename, value w, value h, value comp, value ba) {
  ...
  if (validate_dim(ba, w, h, comp, 4))
    result = stbi_write_hdr(String_val(filename), Int_val(w), Int_val(h), Int_val(comp),
                            Caml_ba_data_val(ba));
  else
    result = 0;
...
CAMLprim value ml_stbi_write_jpg_native(value filename, value w, value h, value comp,
                                        value q, value ba) {
  ...
  if (validate_dim(ba, w, h, comp, 1))
    result = stbi_write_jpg(String_val(filename), Int_val(w), Int_val(h), Int_val(comp),
                            Caml_ba_data_val(ba), Int_val(q));
  else
    result = 0;

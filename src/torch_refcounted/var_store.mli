...
(** A VarStore is used to store all the variables used by a given model. The model creates
    variables by calling [Var_store.new_var] for which it has to provide a name.
    [Var_store.sub] creates a sub-directory in the var store which is useful to group some
    variables together. *)
type t

(** [create ?frozen ?device ~name ()] creates a new variable store on the specified device
    (defaulting to cpu). *)
val create
  :  ?frozen:bool
...
val device : t -> Torch_refcounted_core.Device.t
...
(** [new_var_copy ?trainable t ~src ~name] creates a new variable in [t] by copying tensor
    [src], so using the same shape, element kind and element values. *)
val new_var_copy : ?trainable:bool -> t -> src:Tensor.t @ local -> name:string -> Tensor.t

(** [freeze t] freezes all variables in the var-store, none of the variables are trainable
    anymore and their gradients are not tracked. *)
val freeze : t -> unit

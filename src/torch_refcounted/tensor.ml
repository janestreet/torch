...
let nll_loss ?(reduction = Reduction.Elementwise_mean) xs ~targets =
...
let bce_loss ?(reduction = Reduction.Elementwise_mean) t ~targets =
  binary_cross_entropy t ~target:targets ~weight:None ~reduction
;;

let mse_loss ?(reduction = Reduction.Elementwise_mean) t1 t2 =
  mse_loss t1 ~target:t2 ~reduction
;;

let huber_loss ?(reduction = Reduction.Elementwise_mean) t1 t2 =
...
let bce_loss_with_logits ?(reduction = Reduction.Elementwise_mean) t ~targets =
  with_rc_scope_tensor (fun () ->

include Base
include Torch_refcounted_core.Wrapper.Tensor
...
let type_ = kind
let to_type t ~type_ = exclave_ totype t ~scalar_type:type_
let to_kind t ~kind = exclave_ totype t ~scalar_type:kind

let to_device ?device t =
  match device with
  | None -> t
  | Some device -> exclave_ to_ t ~device
;;

let float_vec ?kind ?device dims = exclave_ float_vec ?kind dims |> to_device ?device
...
let gen
  ...
  let t =
    Option.value_map__local
      scale
      ~f:(fun scale -> exclave_
        let scale = float_vec [ Float.globalize scale ] ~device in
        let result = mul t scale in
        result)
      ~default:t
  in
  let t = if requires_grad then set_requires_grad t ~r:true else t in
  t
;;
...
let f v = exclave_ float_vec [ v ] |> reshape ~shape:[]
let mm = matmul
let ( + ) t t' = exclave_ add t t'
let ( - ) t t' = exclave_ sub t t'
let ( * ) t t' = exclave_ mul t t'
let ( / ) t t' = exclave_ div t t'
let ( ~- ) = neg
...
let const_batch_norm ?(momentum = 0.1) ?(eps = 1e-5) input = exclave_
  batch_norm
    input
    ~weight:None
    ~bias:None
...
let nll_loss ?(reduction = Reduction.Elementwise_mean) xs ~targets = exclave_
  nll_loss xs ~target:targets ~weight:None ~reduction ~ignore_index:(-100)
;;

let cross_entropy_for_logits ?reduction logits ~targets = exclave_
  nll_loss ?reduction (log_softmax logits ~dim:(-1) ~dtype:(T Float)) ~targets
;;

let dropout t ~p ~is_training = exclave_ dropout t ~p ~train:is_training

let bce_loss ?(reduction = Reduction.Elementwise_mean) t ~targets = exclave_
  binary_cross_entropy t ~target:targets ~weight:None ~reduction
;;

let mse_loss ?(reduction = Reduction.Elementwise_mean) t1 t2 = exclave_
  mse_loss t1 ~target:t2 ~reduction
;;

let huber_loss ?(reduction = Reduction.Elementwise_mean) t1 t2 = exclave_
  with_rc_scope_tensor (fun () -> exclave_
    let d = abs (t1 - t2) in
    let half = f 0.5 in
...
let bce_loss_with_logits ?(reduction = Reduction.Elementwise_mean) t ~targets = exclave_
  with_rc_scope_tensor (fun () -> exclave_
...
let pp formatter t =
  ...
  then (
    ...
    Stdlib.Format.pp_print_string formatter (to_string t ~line_size:96);
    Stdlib.Format.pp_print_newline formatter ())
  else
    List.map shape ~f:Int.to_string
    |> String.concat ~sep:", "
    |> Printf.sprintf "Tensor<%s>"
    |> Stdlib.Format.pp_print_string formatter
;;
...
let min_values = amin
let max_values = amax
let minimum t = exclave_ reshape t ~shape:[ -1 ] |> min_values ~dim:[ 0 ] ~keepdim:false
let maximum t = exclave_ reshape t ~shape:[ -1 ] |> max_values ~dim:[ 0 ] ~keepdim:false
let flatten ?(end_dim = -1) t ~start_dim = exclave_ flatten t ~start_dim ~end_dim
let squeeze_last t = exclave_ squeeze_dim t ~dim:(-1)
let scale t f = exclave_ mul_scalar t (Scalar.float f)
let eq_scalar = eq

let eq t1 t2 =
  if Kind.( <> ) (kind t1) (kind t2)
  then false
  else if Stdlib.( <> ) (shape t1) (shape t2)
  then false
...
let to_list t =
  ...
  Base.List.fold__global__local
    ...
    ~init:[]
    ~f:(fun prev_list idx -> exclave_ get t idx :: prev_list)
;;

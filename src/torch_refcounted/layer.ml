...
type t = { apply : Tensor.t @ local -> Tensor.t @ local }

type t_with_training =
  { apply_with_training : Tensor.t @ local -> is_training:bool -> Tensor.t @ local }

let with_training t =
  let apply_with_training xs ~is_training:_ = exclave_ t.apply xs in
  { apply_with_training }
;;
...
let apply ?activation ys = exclave_
...
let linear vs ?activation ?(use_bias = true) ?(w_init @ local) ~input_dim output_dim =
  let w =
...
let batch_norm2d
  ...
  let running_var =
    Var_store.new_var
      ...
      ~name:"running_var"
  in
  let apply_with_training xs ~is_training = exclave_
    Tensor.batch_norm
      xs
      ~weight:(Some w)
      ~bias:(Some b)
...
let forward t xs = exclave_ Tensor.with_rc_scope_tensor (fun () -> exclave_ t.apply xs)

let forward_ t_with_training xs ~is_training = exclave_
  Tensor.with_rc_scope_tensor (fun () -> exclave_
...
let sequential t_list =
  let apply xs = exclave_
    Tensor.with_rc_scope_tensor (fun () -> exclave_
      List.fold__local__local t_list ~init:xs ~f:(fun acc t -> exclave_ t.apply acc))
  in
  { apply }
;;
...
let sequential_ t_list =
  let apply_with_training xs ~is_training = exclave_
    Tensor.with_rc_scope_tensor (fun () -> exclave_
      List.fold__local__local t_list ~init:xs ~f:(fun acc t -> exclave_

...
val of_fn : (Tensor.t @ local -> Tensor.t @ local) -> t

(** [of_fn_ f] creates a layer based on a function from tensors to tensors. [f] also has
    access to the [is_training] flag. *)
val of_fn_ : (Tensor.t @ local -> is_training:bool -> Tensor.t @ local) -> t_with_training
...
val forward : t -> Tensor.t @ local -> Tensor.t @ local

(** [forward_ t tensor ~is_training] applies layer [t] to [tensor] with the specified
    [is_training] flag. *)
val forward_ : t_with_training -> Tensor.t @ local -> is_training:bool -> Tensor.t @ local

...
let read_files ?(dirname = "data") ?(with_caching = false) () =
  let read_one filename = Stdlib.Filename.concat dirname filename |> read_file in
  let read () =
    ...
    { Dataset_helper.train_images = Tensor.concat train_images ~dim:0
    ; train_labels = Tensor.concat train_labels ~dim:0
    ; test_images
    ; test_labels
    }
  in
  if with_caching
  then (
    let cache_file = Stdlib.Filename.concat dirname "cifar-cache.ot" in

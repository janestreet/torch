...
raw_tensor get_raw_tensor(value managed_tensor) {
  return *(raw_tensor *)Data_custom_val(managed_tensor);
}

void increment_refcount_internal(value managed_tensor) {
  raw_tensor t = get_raw_tensor(managed_tensor);
...
void decrement_refcount_internal(value managed_tensor) {
  // Get an intrusive ptr without incrementing and let it drop
  raw_tensor t = get_raw_tensor(managed_tensor);
  auto ptr =
...
int get_refcount_internal(value managed_tensor) {
  raw_tensor t = get_raw_tensor(managed_tensor);
...
#include "torch_refcounted_api_generated.cpp"
